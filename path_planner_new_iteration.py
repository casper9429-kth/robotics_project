...
from geometry_msgs.msg import PoseStamped, PoseArray,Quarternion
...
class A_star():
        ...
        for longditude in range(-1, 2, 1):
            for latitude in range(-1, 2, 1):
                ...
                if self.is_in_bounds(neighbour):
                    ...
                    # will always work due to checking inbounds
                    if self.get_value_of_pos(new_x, new_y) >= (0.8):
                        neighbour.g = np.inf
    ...
    def get_robot_pose_in_map(self):
        try:
            transform_base_link_2_map = self.tfBuffer.lookup_transform('map', 'base_link', self.t_stamp, rospy.Duration(
                0.5))  # lookup_transform('target frame','source frame', time.stamp, rospy.Duration(0.5))
            self.robot_pose = tf2_geometry_msgs.do_transform_pose(
                self.pose, transform_base_link_2_map)
    ...
    def path(self, goal):
        ...
        rospy.loginfo(
            f'Path planner: start {start.pose.position.x} {start.pose.position.y}')
        rospy.loginfo(
            f'Path:planner: goal {goal.pose.position.x,goal.pose.position.y}')
        if start == None:
            return False, []
        else:
        ...
        while heapq and iter < self.iterations:
            current_pos = min(
                openset, key=lambda cordinates: openset[cordinates].f)
            try:
            ...
            if abs(current.x-current.goal[0]) <= self.goal_tolerance and abs(current.y-current.goal[1]) <= self.goal_tolerance:
                print(
                    f'diff x {current.x-current.goal[0]}, diffy {current.y-current.goal[1]}')
...
class Path_Planner():
    def __init__(self) -> None:
        ...
        self.start_and_goa_sub = rospy.Subscriber(
            '/start_and_goal', PoseStamped, self.start_and_goal_callback)
        self.goal_reached_sub = rospy.Subscriber(
            '/goal_reached', Bool, self.goal_reached_callback)
...
############################################ Main ############################################
    ...
    # TODO fix oscilation problem
    # in practice it takes to long for robot to move to point that it might just be worth it to send the next point
    ...
    def send_path(self, path):  # TODO  fix to publish
        ...
        if self.last_msg != path_list[0]:
            ...
            if self.reached_goal == True:
                ...
                self.reached_goal = False
            print(
                f'Path planner: Published point {path_list[0].pose.position.x,path_list[0].pose.position.y  }')
            self.move_to_pub.publish(path_list[0])
